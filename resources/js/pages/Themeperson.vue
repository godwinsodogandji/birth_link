<template>
  <div class="flex items-center justify-center min-h-screen p-6 bg-gray-100">
    ...
    <div class="space-y-6 mr-20">
      <button @click="showCard(1)" class="bg-red-500 text-white py-2 px-4 rounded w-full hover:bg-red-600 transition">Carte 1</button>
    </div>

    <!-- Container for Cards -->
    <section class="grid grid-cols-1 gap-4 ml-8">
      <div v-show="isCardVisible" class="card bg-white p-6 rounded-lg shadow-md text-center flex items-center justify-center flex-col bg-cover" :style="{ minHeight: '990px', minWidth: '850px', backgroundImage: `url(${backgroundImage})` }">
        <div class="bg-white bg-opacity-75 p-4 rounded-lg shadow-lg">
          <!-- Title with dynamic color -->
          <h2 :style="{ color: titleColor }" class="font-bold text-4xl mb-4">{{ cardTitle }}</h2>
          <textarea class="bg-transparent text-gray-700 font-semibold text-xl text-center w-full h-full resize-none focus:outline-none" rows="10" readonly>
            {{ cardText }}
          </textarea>
        </div>
        <div class="mt-4 space-x-4">
          <button @click="showForm" class="bg-red-500 text-white py-2 px-4 rounded hover:bg-red-600 transition">Personnaliser le texte</button>
          <button @click="sendText" class="bg-red-500 text-white py-2 px-4 rounded hover:bg-red-600 transition">Envoyer</button>
        </div>
      </div>
    </section>

    <!-- Modal for Confirmation -->
    <div v-show="isConfirmationModalVisible" class="fixed inset-0 bg-gray-800 bg-opacity-50 flex items-center justify-center">
      ...
      <div class="bg-white p-6 rounded-lg shadow-lg text-center">
        ...
        <form @submit.prevent="saveCustomization">
          <div class="mb-4">
            ...
            <input v-model="customTitle" type="text" id="title" name="title" class="w-full p-2 border border-gray-300 rounded" required>
          </div>
          <div class="mb-4">
            <label for="message" class="block text-gray-700">Message</label>
            <textarea v-model="customMessage" id="message" name="message" class="w-full p-2 border border-gray-300 rounded" rows="4" required></textarea>
          </div>
          <div class="mb-4">
            <label for="photo" class="block text-gray-700">Photo</label>
            <input type="file" @change="handleImageChange" id="photo" name="photo" class="w-full p-2 border border-gray-300 rounded">
          </div>
          <div class="mb-4">
            <label for="color" class="block text-gray-700">Couleur du titre</label>
            <input v-model="titleColor" type="color" id="color" name="color" class="w-full p-2 border border-gray-300 rounded">
          </div>
          <div class="space-x-4">
